import { useRouter } from "next/navigation";
import React from "react";

const Custom404 = () => {
    const router = useRouter();
  return (
    <div className="flex h-[100vh] justify-center items-center">
      <div className="text-center">
        <h1 className="font-bold text-red text-[100px]">Opps!</h1>
        <h1 className="font-bold text-red text-[80px]">Page not fount</h1>
        <button className="px-[20px] py-[5px] bg-blue rounded-[8px] text-white mt-4 font-medium text-[18px]" onClick={()=>router.push('/')}>
          <div className="flex gap-2 items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              width="22"
              height="22"
            >
              <path
                d="M19 21H5C4.44772 21 4 20.5523 4 20V11L1 11L11.3273 1.6115C11.7087 1.26475 12.2913 1.26475 12.6727 1.6115L23 11L20 11V20C20 20.5523 19.5523 21 19 21ZM6 19H18V9.15745L12 3.7029L6 9.15745V19ZM9 9.99998H15V16H9V9.99998ZM11 12V14H13V12H11Z"
                fill="rgba(255,255,255,1)"
              ></path>
            </svg>
            Go to home
          </div>
        </button>
      </div>
    </div>
  );
};

export default Custom404;
